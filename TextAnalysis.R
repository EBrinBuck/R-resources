...
tstat_freq
ggplot(tstat_freq, aes(x = reorder(feature, frequency), y = rank)) +
  facet_wrap(~group)+
  geom_point() +
  coord_flip() +
  labs(x = NULL, y = "Rank") + ggtitle("Top 10 words by decade")+
  theme_minimal()+ scale_y_continuous(breaks=c(1,5,10))
